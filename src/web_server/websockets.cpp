...
void basic_web_server<T>::websocket_accept_read_cb(const std::string &sec_websocket_key, const std::string &path, int client_idx) {
  ...
  while ((path_part = strtok_r(path_dup, "/", &save_ptr)) != nullptr) {
    path_dup = nullptr;
    subdirs.emplace_back(path_part);
  }
    ...
    for (auto &frame : frames) {
      if (!frame.empty()) {
        ...
        if (processed_data.first == 1) { // 1 is to indicate that it's done
          if (!client_data.websocket_frames.empty()) {
...
template <server_type T>
auto basic_web_server<T>::websocket_process_write_cb(int client_idx) -> bool {
  auto ws_client_idx = tcp_clients[client_idx].ws_client_idx;
  if (all_websocket_connections.count(ws_client_idx) != 0U) {
...
template <server_type T>
auto basic_web_server<T>::new_ws_client(int client_idx) -> int {
  auto index = 0;

  if (!freed_indexes.empty()) {     //if there's a free index, give that
...
template <server_type T>
auto basic_web_server<T>::get_accept_header_value(std::string input) -> std::string {
  input += "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"; //concatenate magic string
  std::array<unsigned char, SHA_DIGEST_LENGTH> hash{};
  SHA1(reinterpret_cast<const unsigned char *>(input.c_str()), input.size(), hash.data());
  //for input of length 20, base64 output length is 28 (a bit over *4/3 - *4/3 for larger lengths)
  std::string base64data;
  constexpr uint32_t hash_data_size = 28;
  base64data.resize(hash_data_size); //no need for +1, std::string does that for you
  EVP_EncodeBlock(reinterpret_cast<unsigned char *>(base64data.data()), hash.data(), SHA_DIGEST_LENGTH);
  return base64data;
}

template <server_type T>
auto basic_web_server<T>::get_ws_frame_length(const char *buffer) -> ulong {
  constexpr uint32_t MAX_SMALL_PACKET = 127;

  ulong packet_length = buffer[1] & 0x7f;
  if (packet_length == MAX_SMALL_PACKET) {
...
template <server_type T>
auto basic_web_server<T>::make_ws_frame(const std::string &packet_msg, websocket_non_control_opcodes opcode) -> std::vector<char> {
...
template <server_type T>
auto basic_web_server<T>::close_ws_connection_potential_confirm(int ws_client_idx) -> bool {
  auto &client_data = websocket_clients[ws_client_idx];
...
std::pair<int, std::vector<char>> basic_web_server<T>::decode_websocket_frame(std::vector<char> &&data) {
  ...
  if (!mask)
    return {-1, {}}; //mask must be set
  if (opcode == 0x8)
    return {-3, {}}; //the close opcode
  if (opcode == 0xA)
    return {3, {}}; //the pong opcode
  ...
  if (opcode == websocket_non_control_opcodes::ping)
    return {2, data}; //the ping opcode
  if (!fin)
    return {-2, data}; //fin bit not set, so put this in a pending larger buffer of decoded data
